...
for j in "${!delta[@]}"; do
    ...
    for i in "${!temps[@]}"; do
        ...
        base="${SCRATCH}/projects/crystal_fem/coexistence/${model}"
        pattern="coexistence_T_${temps[$i]}_P_${press[$i]}_delta_${delta[$j]}_*"

        first_match=( "$base"/$pattern/ )
        folderpath="${first_match[0]}"

        if [ ! -d "$folderpath" ]; then
            echo "Path $folderpath does not exist"
            exit 1
        fi
        ...
        lmp -in lammps/continue_coexistence.lmp \
