...
class DifftreParallel(tt.MLETrainerTemplate):
    ...
    def __init__(self,
        ...
        self.pair_batches = pair_batches
    ...
    def _get_batch(self):
        ...
        if self.pair_batches:
            # Only select the first half of the statepoints
            batches = random.permutation(split, num_statepoints // 2).reshape(
                (-1, self.batch_size // 2)
            )
            assert onp.all(batches < num_statepoints // 2)

            for b in batches:
                # Return the corresponding pairs
                yield jnp.concatenate([b, b + num_statepoints // 2], axis=0)
        else:
